...
namespace _58.Array.TwoDimension.Exercise.SelectionSorting
{
    class Program
    {
        ...
        private static void SortArray2()
        {
            ...
            {
                var current = MovePosition(i);
                var min = current;

                for(int j = i + 1; j < numbers.Length; j++)
                {
                    var temp = MovePosition(j);
            ...
            //Local Method
            (int row, int column) MovePosition(int position)
                => (position / numbers.GetLength(1), position % numbers.GetLength(1));

        }

        //private static (int row, int column) MovePosition(int position, int[,] arr)
